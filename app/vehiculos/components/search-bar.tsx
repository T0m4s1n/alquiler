      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className={`relative flex-1 transition-all duration-300 ${isFocused ? "transform -translate-y-1" : ""}`}>
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search
              size={20}
              className={`transition-colors duration-300 ${isFocused ? "text-[#333]" : "text-gray-400"}`}
            />
          </div>
          <input
            type="text"
            placeholder="Buscar por marca, modelo, matrícula o tipo..."
        ...
        <div>
          <p className="text-gray-600">
            ...
            {Math.min(currentPage * itemsPerPage, totalItems)} de {totalItems} vehículos
          </p>
        </div>
      </div>
    </div>
