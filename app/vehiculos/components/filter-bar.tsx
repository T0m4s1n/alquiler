...
import type { FilterType, FilterOption } from "../types/vehicle"
...
export default function FilterBar({ onFilter, onClearFilter, isFiltering }: FilterBarProps) {
  ...
  const handleFilterTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Si selecciona "disponible", aplicar el filtro automáticamente
    if (e.target.value === "disponible") {
      onFilter("disponible", "true")
    }
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col md:flex-row md:items-end gap-4">
        ...
        <div className={`flex-1 transition-all duration-300 ${isFocused ? "transform -translate-y-1" : ""}`}>
          ...
          <div className="relative">
            ...
            <input
              ...
              disabled={filterType === "all" || filterType === "disponible"}
            />
          </div>
        </div>
        ...
        <div className="flex space-x-2">
          <button
            ...
            disabled={(filterType === "all" && !filterValue.trim()) || filterType === "disponible"}
          >
